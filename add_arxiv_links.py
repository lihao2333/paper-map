...
# -*- coding: utf-8 -*-
"""
使用 arXiv API 搜索论文链接并添加到 XML 文件中
"""
...
def update_xml_with_arxiv_links(xml_path):
    ...
    for record in root.find('records').findall('record'):
        ...
        for item in record.findall('item'):
            ...
            # 提取论文标题（通常在冒号之前，或者整个文本）
            title_match = re.match(r'^([^:]+?):', text_content)
            if title_match:
                paper_title = title_match.group(1).strip()
            else:
                # 如果没有冒号，使用前几个单词作为标题
                words = text_content.split()
                paper_title = ' '.join(words[:5])
            
            print(f"\n搜索: {paper_title}")
            print(f"当前时间: {current_identifier}")
            
            # 搜索 arXiv
            arxiv_response = search_arxiv(paper_title)
            if not arxiv_response:
                print("  未找到 arXiv 结果")
                time.sleep(1)  # 避免请求过快
                continue
            
            match = parse_arxiv_response(arxiv_response, paper_title)
            if not match:
                print("  未找到匹配的论文")
                time.sleep(1)
                continue
