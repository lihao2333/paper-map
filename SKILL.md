...
| **completer.py** | 论文缓存/元数据补全（可原子步骤分跑） | `python completer.py --help`（`arxiv:comment` 与 `venue` 分步：`--only arxiv_comment` 再 `--only venue`；`arxiv_metadata` 一次 API 写摘要/作者/标题/comment） |
